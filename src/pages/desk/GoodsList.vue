...
export default {
  ...
  methods:{
    ...
    moreList(){
      this.page++;
      var url = this.local + '/api/desk/sp_list';
      var data = {
        userid: localStorage.getItem("token"),
        page: this.page,
        limit: this.limit
    ...
    jumpBilling(item,index){
      if(!this.isMy){
        this.$router.push({path:'/desk_edit_goods',query:{itemGoodsData:item,itemGoodsIndex:index}});
      }else{
        sessionStorage.setItem("goodsStatus","edit");
        this.$router.push({path:'/desk_add_goods?isMy=true',query:{itemGoodsData:item}});
      }
    },
    //下拉加载更多
    loadBottom(){
    ...
    jumpAddGoods(){
      ...
      this.$router.push({path:'/desk_add_goods?isMy=true'});
    },
    goback(){
      if(this.isMy){
