...
export default {
  ...
  beforeRouteEnter(to, from, next){
    ...
    }else if(from.name=='DeskBilling'||from.name=='DeskBillingCode'){
  ...
  methods:{
      ...
      if(this.isDone){ //通过了
        ...
        this.timer = setTimeout(() => {
          if(this.modeType=='1'){  //手填
            if(this.changeStatus){
              this.$router.push({path:'/desk_billing',query:{itemGoodsData: this.data,changeStatus:'true',index:this.index,return:'true'}});
            }else{
              this.$router.push({path:'/desk_billing',query:{itemGoodsData: this.data,return:'true'}});
            }
          }else if(this.modeType=='0'){  //开票
            if(this.changeStatus){
              this.$router.push({path:'/desk_billing_code',query:{itemGoodsData: this.data,changeStatus:'true',index:this.index,return:'true'}});
            }else{
              this.$router.push({path:'/desk_billing_code',query:{itemGoodsData: this.data,return:'true'}});
            }
          }
        }, 500)
      }
    ...
    deleteData(){
      ...
      this.timer = setTimeout(() => {
        if(this.modeType=='1'){  //手填
          this.$router.push({path:'/desk_billing',query:{deleteStatus:'true',index:this.index}});
        }else if(this.modeType=='0'){  //开票
          this.$router.push({path:'/desk_billing_code',query:{deleteStatus:'true',index:this.index}});
        }
      }, 500)
    },
    //表单验证
    ...
    //跳转商品列表
    jumpGoodsList(){
      this.$router.push({path:'/desk_goods_list'});
    },
    //返回上一页
    goback(){
      if(this.modeType=='1'){  //手填
        this.$router.push({path: '/desk_billing'});
      }else if(this.modeType=='0'){  //开票
