<template>
  <view-box ref="viewBox" class="electronic_detail">
    ...
    <ul class="commonList">
      <li>
        <p class="leftCon">小票订单号</p>
        <p class="rightCon overflowCon">{{data.code||'暂无'}}</p>
      </li>
      <li>
    ...
    <div class="maskImg" v-show="viewStatus">
      <img :src="data.url" ref="printImg" id="printImg" />
      <span class="maskClose" @click="toClose">
        <img src="../../assets/icon_close.png" />
      </span>
      <!--<button class="commonBtn" v-print="'#printImg'">打印</button>-->
    </div>
...
import Qrcode from 'vux/src/components/qrcode/index'
  ...
  methods:{
    ...
    send(){
      ...
      if(this.isDone){  //验证通过
        ...
        var url = this.local+'/api/desk/sendpdf';
        var data = {
