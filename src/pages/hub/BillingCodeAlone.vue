  ...
  methods:{
    ...
    changeOption(event){
      ...
      if(this.billingType=='two'){
        this.$refs.two_spdj.value = this.sp_list[index].spdj;
      }
      if(this.billingType=='three'){
        this.$refs.three_spdj.value = this.sp_list[index].spdj;
      }
    },
    //弹窗显示
    showPopups(){
        ...
        if(response.errcode==0){
          ...
          if(response.sp_list.length>0){
            ...
            if(_this.billingType=='two'){
              _this.$refs.two_spdj.value = _this.sp_list[0].spdj;
            }
            if(_this.billingType=='three'){
              _this.$refs.three_spdj.value = _this.sp_list[0].spdj;
            }
          }else{
    ...
    submit(){
      ...
      if(this.$refs.spje.value){
        if(Number(this.$refs.spje.value)<0.01){
          this.popupsStatus = true;
          this.showPopups();
          this.text = '请正确填写含税金额';
          this.isDone = false;
