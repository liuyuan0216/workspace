<template>
  <view-box ref="viewBox" class="goods_classify_sub_next1">
    <x-header
      slot="header"
      class="header"
...
export default {
  ...
  data(){
    return{
      ...
      name_sub: this.$route.query.itemData.name
    }
  },
  components:{
    ViewBox,
  ...
  methods:{
    jumpNext(data){
      if(data.children){
        this.$router.push({path:'/goods_classify_sub_next2',query:{itemData: data,name:this.name,name_sub:this.name_sub}});
      }else{
...
<style scoped>
  .goods_classify_sub_next1 .leftCon{
    width:100%;
    display: flex;
    align-items: center;
