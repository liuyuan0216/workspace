<template>
  <view-box ref="viewBox" class="goods_classify_sub_next3">
    <x-header
      slot="header"
      class="header"
    ...
    <div class="breadcrumbs">
      ...
      <p class="activeCrumbs">&gt;</p>
      <p class="activeCrumbs thirdCrumbs overflowCon" @click="returnSubNext">{{name_sub_next1}}</p>
      <p class="activeCrumbs">&gt;</p>
      <p class="lastCrumbs overflowCon">{{name_sub_next2}}</p>
    </div>
    <ul class="commonList">
...
export default {
  ...
  data(){
    return{
      ...
      name_sub_next2: this.$route.query.itemData.name
    }
  },
  components:{
    ViewBox,
  ...
  beforeRouteEnter(to, from, next){
    if(from.name=='GoodsClassifySubNext2'){
      next(vm => {
        ...
        vm.name_sub_next2 = vm.$route.query.itemData.name;
      });
    }else{
      next();
    }
  ...
  methods:{
    jumpNext(data){
      if(data.children){
        this.$router.push({path:'/goods_classify_sub_next4',query:{itemData: data,name:this.name,name_sub:this.name_sub,name_sub_next1:this.name_sub_next1,name_sub_next2:this.name_sub_next2}});
      }else{
...
<style scoped>
  .goods_classify_sub_next3 .leftCon{
    width:100%;
    display: flex;
    align-items: center;
