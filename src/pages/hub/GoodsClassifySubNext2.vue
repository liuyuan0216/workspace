<template>
  <view-box ref="viewBox" class="goods_classify_sub_next2">
    <x-header
      slot="header"
      class="header"
    ...
    <div class="breadcrumbs">
      ...
      <p class="activeCrumbs">&gt;</p>
      <p class="lastCrumbs overflowCon">{{name_sub_next}}</p>
    </div>
    <ul class="commonList">
...
export default {
  name: 'GoodsClassifySubNextNext2',
  data(){
    return{
      ...
      name_sub_next: this.$route.query.itemData.name
    }
  },
  components:{
    ViewBox,
  ...
  beforeRouteEnter(to, from, next){
    if(from.name=='GoodsClassifySubNext1'){
      next(vm => {
        ...
        vm.name_sub_next = vm.$route.query.itemData.name;
      });
    }else{
      next();
    }
  ...
  methods:{
    jumpNext(data){
      if(data.children){
        this.$router.push({path:'/goods_classify_sub_next3',query:{itemData: data,name:this.name,name_sub:this.name_sub,name_sub_next1:this.name_sub_next}});
      }else{
...
<style scoped>
  .goods_classify_sub_next2 .leftCon{
    width:100%;
    display: flex;
    align-items: center;
